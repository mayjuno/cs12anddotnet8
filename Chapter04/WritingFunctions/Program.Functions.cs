...
partial class Program
{
    ...
    /// <summary>
    /// pass a 32-bit unsigned integer and it will be convert into its
    /// oridinal equavilent.
    /// </summary>
    ...
    static int Factorial(int number)
    {
        if (number < 0)
        {
            throw new ArgumentOutOfRangeException(message:
                ...
                $"Input:{number}",paramName: nameof(number));
        }
        else if (number == 0)
        {
            return 1;
    ...
    static void RunFactorial()
    {
        for (int i = 1; i <=15; i++)
        {
            ...
            {
                Console.WriteLine($"{i}! throw {ex.GetType()}: {ex.Message}");
            }
    ...
    // FibFunctional written in a declarative
